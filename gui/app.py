...
def login_window():
    ...
    Label(frame, bg="green", text="Pyflora Container", font=("Arial", 28)).pack(anchor="ne", expand=True, fill=X,
                                                                                side=TOP, ipady=5)
...
def store_addnew_herb():  # za dodavanje u bazu db.add_herbs-radi, isključena je samo zbog isprobavanja drugih funkcija
    ...
    if remove(
            input_herb_name.get() and input_soil_moisture.get() and input_luminosity.get() and input_air_temperature.get() and input_ph_value.get()
        ...
        db.add_herbs(conn=conn, name=input_herb_name.get(), soil_moisture=input_soil_moisture.get(),
                     luminosity=input_luminosity.get(), air_temperature=input_air_temperature.get(),
                     ph_value=input_ph_value.get(), features=input_features.get(), herb_hight=input_herb_height.get(),
                     herb_width=input_herb_width.get(), image=herb_image.get())
    input_herb_name.set("")
...
def addnew_herb(event):
    ...
    Entry(frame_2, textvariable=input_herb_name, background="white", fg="black", width=50).grid(row=2, column=0, pady=5,
                                                                                                padx=50)
    Label(frame_2, text="Add soil moisture", fg="red").grid(row=3, column=0, sticky="w", padx=50)
    Entry(frame_2, textvariable=input_soil_moisture, background="white", fg="black", width=50).grid(row=4, column=0,
                                                                                                    pady=5, padx=50)
    Label(frame_2, text="Add luminosity", fg="red").grid(row=5, column=0, sticky="w", pady=5, padx=50)
    Entry(frame_2, textvariable=input_luminosity, background="white", fg="black", width=50).grid(row=6, column=0,
                                                                                                 pady=5, padx=50)
    Label(frame_2, text="Add herb height", fg="red").grid(row=7, column=0, sticky="w", pady=5, padx=50)
    Entry(frame_2, textvariable=input_herb_height, background="white", fg="black", width=50).grid(row=8, column=0,
                                                                                                  pady=5, padx=50)
    get_images()
    ...
    Entry(frame_2, textvariable=input_air_temperature, background="white", fg="black", width=50).grid(row=2, column=1,
                                                                                                      pady=5, padx=20)
    Label(frame_2, text="Add ph value", fg="red").grid(row=3, column=1, sticky="w", pady=5, padx=20)
    Entry(frame_2, textvariable=input_ph_value, background="white", fg="black", width=50).grid(row=4, column=1, pady=5,
                                                                                               padx=20)
    Label(frame_2, text="Add features", fg="red").grid(row=5, column=1, sticky="w", pady=5, padx=20)
    Entry(frame_2, textvariable=input_features, background="white", fg="black", width=50).grid(row=6, column=1, pady=5,
                                                                                               padx=20)
    Label(frame_2, text="Add herb width", fg="red").grid(row=7, column=1, sticky="w", pady=5, padx=20)
    Entry(frame_2, textvariable=input_herb_width, background="white", fg="black", width=50).grid(row=8, column=1,
                                                                                                 padx=20)
...
def store_update_herb():
    ...
    def edit_herb():
        db.update_herb(conn, var_herb_name.get(), var_herb_moisture.get(), var_herb_luminosity.get(),
                       var_herb_air_temp.get(), var_herb_ph.get(), var_herb_features.get(),
...
def update_herb(event):
    ...
    frame_2 = Label(tp)
    frame_2.pack(fill=BOTH, expand=True, padx=160)
    Label(frame_2, textvariable=edit_herb_name, fg="green", font=("Arial", 20)).grid(row=0, column=0, sticky="w",
                                                                                     pady=50, padx=50)
    Label(frame_2, text="Change herb name", fg="red").grid(row=1, column=0, sticky="w", pady=5, padx=50)
    Entry(frame_2, textvariable=edit_herb_name, background="white", fg="black", width=50).grid(row=2, column=0, pady=5,
                                                                                               padx=50)
    Label(frame_2, text="Change soil moisture", fg="red").grid(row=3, column=0, sticky="w", pady=5, padx=50)
    Entry(frame_2, textvariable=edit_soil_moisture, background="white", fg="black", width=50).grid(row=4, column=0,
                                                                                                   pady=5, padx=50)
    Label(frame_2, text="Change luminosity", fg="red").grid(row=5, column=0, sticky="w", pady=5, padx=50)
    Entry(frame_2, textvariable=edit_luminosity, background="white", fg="black", width=50).grid(row=6, column=0, pady=5,
                                                                                                padx=50)
    ...
    get_images()
    Button(frame_2, text="Change herb image", fg="green", command=add_herb_photo).grid(row=9, column=0, sticky="w",
                                                                                       padx=50, pady=5)
    Entry(frame_2, textvariable=edit_herb_height, background="white", fg="black", width=50).grid(row=8, column=0,
                                                                                                 pady=5, padx=50)
    Label(frame_2, text="Change air temperature", fg="red").grid(row=1, column=1, sticky="w", padx=20)
    Entry(frame_2, textvariable=edit_air_temperature, background="white", fg="black", width=50).grid(row=2, column=1,
                                                                                                     sticky="w",
                                                                                                     padx=20)
    Label(frame_2, text="Change ph value", fg="red").grid(row=3, column=1, sticky="w", padx=20)
    Entry(frame_2, textvariable=edit_ph_value, background="white", fg="black", width=50).grid(row=4, column=1, padx=20,
                                                                                              sticky="w")
    Label(frame_2, text="Change features", fg="red").grid(row=5, column=1, sticky="w", padx=20)
    Entry(frame_2, textvariable=edit_features, background="white", fg="black", width=50).grid(row=6, column=1, padx=20,
                                                                                              sticky="w")
    Label(frame_2, text="Change herb width", fg="red").grid(row=7, column=1, sticky="w", padx=20)
    Entry(frame_2, textvariable=edit_herb_width, background="white", fg="black", width=50).grid(row=8, column=1,
                                                                                                padx=20)
...
def details_pyflora_container(event, container_id):
    ...
    if container_herb_id is None:
        ...
        herbs_id, herb_name, soil_moisture, luminosity, air_temperature, ph_value, features, herb_height, herb_width, image = db.get_herb(
            conn, container_herb_id)
        var_herb_image = image
    Label(frame_2, textvariable=var_container_name, fg="green", font=("Arial", 20)).grid(row=0, column=0, sticky=W,
                                                                                         pady=10, padx=136)
    Button(frame_2, text="UPDATE", fg="green", command=update_pycontainers, width=12).grid(row=0, column=1, sticky="w",
                                                                                           ipady=2, pady=10, padx=470)
    Button(frame_2, text="DELETE", fg="red", width=12, command=delete_button_pycontainer).grid(row=0, column=1,
                                                                                               sticky="w", ipady=2,
                                                                                               pady=10, padx=307)
...
def details_herb(event, herb_id):
    ...
    var_herb_image = image
    Label(frame_2, textvariable=var_herb_name, fg="green", font=("Arial", 20)).grid(row=0, column=0, padx=200,
                                                                                    sticky="w")
    Button(frame_2, text="UPDATE", fg="green", command=update_herbs, width=12, height=1).grid(row=0, column=1, ipady=2,
                                                                                              sticky=W, pady=20,
                                                                                              padx=580)
    Button(frame_2, text="DELETE", fg="red", width=12, command=delete_button_herb).grid(row=0, column=1, sticky=W,
                                                                                        ipady=2, pady=20, padx=420)
    canvas = Canvas(frame_2, width=300, height=300, bg="SpringGreen2")
...
def edit_profile_screen():
    ...
    def update_profile():
        ...
        if remove(new_name) == "" or remove(new_lastname) == "" or remove(new_password) == "":
            ...
            return
        update_data()
        messagebox.showinfo(tp, message=f"User {user} successfully changed data!")
        tp.withdraw()
        my_profile()
